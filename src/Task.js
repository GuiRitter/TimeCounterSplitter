	...
	render() {
		...
		return buildRow(
			...
			buildCell(this.buildKey('active'), this.props.task.active
				? 'active'
				: ''),
			buildCell(this.buildKey('count'), floor(count, 1), { onClick: () => alert(count), title: count }),
			buildCell(this.buildKey('proportional'), this.props.task.ignored ? 'ignored' : floor(proportional, 1), { onClick: () => alert(proportional), title: proportional }),
